...
@Component({
  ...
  template: `
    <div class="p-4">
      <h2 class="mb-4 text-xl font-bold">Teilnehmer verwalten</h2>
      <mat-list>
        <mat-list-item
          *ngFor="let participant of sortedParticipants"
          class="participant-item"
        >
          <div class="flex w-full items-center justify-between">
            <div class="participant-info flex-1 overflow-hidden">
              {{ participant.invitee.email }} -
...
export class ParticipantsDialogComponent {
  ...
  get sortedParticipants(): AccountInvitationDetailsDTO[] {
    ...
    return this.accountInvitationDetailsDTO.sort((a, b) =>
      this.compareStatus(a.status, b.status)
    );
  }
  ...
  uninviteParticipant(participant: AccountInvitationDetailsDTO) {
    if (this.accountInvitationDetailsDTO && this.eventId != undefined) {
      this.eventHostService
        .removeParticipant(this.eventId, participant.id.toString())
        .subscribe({
          next: () => {
            this.accountInvitationDetailsDTO =
              this.accountInvitationDetailsDTO?.filter(
