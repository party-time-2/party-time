...
const groupName = 'Events bearbeiten';
describe(groupName, () => {
  beforeEach(() => {
    cy.visit('/');
  });

  const eventUpdateSuccess = 'erstelltes Event bearbeiten';
  it(eventUpdateSuccess, () => {

    cy.screenshot(
      getScreenshotPath(requirement, groupName, eventUpdateSuccess),
      {
        overwrite: true,
      }
    );
